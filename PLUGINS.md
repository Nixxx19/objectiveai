...
3. **(Optional) Build a viewer bundle.** A static `dist/` from your favourite frontend stack (vite, plain HTML, anything). Use the [`@objectiveai/plugin-sdk`](objectiveai-plugin-sdk/) for `invoke()` / `listen()` to talk to the host. See the [viewer plugins reference](objectiveai-viewer/PLUGINS.md).
