...
3. **(Optional) Build a viewer bundle.** A static `dist/` from your favourite frontend stack (vite, plain HTML, anything). Use [`@objectiveai/plugin-sdk`](objectiveai-plugin-sdk/)'s `listen()` to receive events from the host. See the [viewer plugins reference](objectiveai-viewer/PLUGINS.md).
