    ...
    unsafe {
        run(json_out, json_out_len, || {
            let fields: objectiveai::functions::check::VectorFieldsValidation =
                from_json(json_in, json_in_len)?;
            objectiveai::functions::check::check_vector_fields(fields, None).map_err(|e| e)?;
            Ok(Vec::new())
        })
    }
}
    ...
    unsafe {
        run(json_out, json_out_len, || {
            let fields: objectiveai::functions::check::ScalarFieldsValidation =
                from_json(json_in, json_in_len)?;
            objectiveai::functions::check::check_scalar_fields(fields, None).map_err(|e| e)?;
            Ok(Vec::new())
        })
    }
}
    ...
    unsafe {
        run(json_out, json_out_len, || {
            let function: objectiveai::functions::alpha_scalar::RemoteFunction =
                from_json(json_in, json_in_len)?;
            objectiveai::functions::alpha_scalar::check::check_alpha_leaf_scalar_function(
                &function, None,
            )
            .map_err(|e| e)?;
            Ok(Vec::new())
    ...
    unsafe {
        run(json_out, json_out_len, || {
            let function: objectiveai::functions::alpha_vector::RemoteFunction =
                from_json(json_in, json_in_len)?;
            objectiveai::functions::alpha_vector::check::check_alpha_leaf_vector_function(
                &function, None,
            )
            .map_err(|e| e)?;
            Ok(Vec::new())
...
pub unsafe extern "C" fn objectiveai_vector_completion_chunk_to_unary(
    ...
    unsafe {
        run(json_out, json_out_len, || {
            let a: objectiveai::vector::completions::response::streaming::VectorCompletionChunk =
                from_json(json_in, json_in_len)?;
            let unary: objectiveai::vector::completions::response::unary::VectorCompletion =
                a.into();
            to_json(&unary)
        })
    }
...
pub unsafe extern "C" fn objectiveai_function_execution_chunk_to_unary(
    ...
    unsafe {
        run(json_out, json_out_len, || {
            let a: objectiveai::functions::executions::response::streaming::FunctionExecutionChunk =
                from_json(json_in, json_in_len)?;
            let unary: objectiveai::functions::executions::response::unary::FunctionExecution =
                a.into();
            to_json(&unary)
        })
    }
...
pub unsafe extern "C" fn objectiveai_function_invention_chunk_to_unary(
    ...
    unsafe {
        run(json_out, json_out_len, || {
            let a: objectiveai::functions::inventions::response::streaming::FunctionInventionChunk =
                from_json(json_in, json_in_len)?;
            let unary: objectiveai::functions::inventions::response::unary::FunctionInvention =
                a.into();
            to_json(&unary)
        })
    }
...
pub unsafe extern "C" fn objectiveai_function_invention_recursive_chunk_to_unary(
    ...
    unsafe {
        run(json_out, json_out_len, || {
            let a: objectiveai::functions::inventions::recursive::response::streaming::FunctionInventionRecursiveChunk =
                from_json(json_in, json_in_len)?;
            let unary: objectiveai::functions::inventions::recursive::response::unary::FunctionInventionRecursive =
                a.into();
            to_json(&unary)
        })
    }
