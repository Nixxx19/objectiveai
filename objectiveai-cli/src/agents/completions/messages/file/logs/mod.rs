    ...
    pub async fn handle(self, cli_config: &crate::Config) -> Result<crate::Output, crate::error::Error> {
        ...
        match self {
            Commands::Get { id, message_index, media_index } => {
                let content = client.read_agent_completion_message_file(&id, message_index, media_index).await.map(objectiveai::filesystem::logs::LogContent::DataUrl)?;
                Ok(crate::Output::LogsGet(content))
            }
            Commands::Subscribe { id, message_index, media_index, timeout_ms } => {
                let result = client.subscribe_agent_completion_message_file(&id, message_index, media_index, std::time::Duration::from_millis(timeout_ms)).await;
