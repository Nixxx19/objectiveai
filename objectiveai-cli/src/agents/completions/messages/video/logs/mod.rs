    ...
    pub async fn handle(self, cli_config: &crate::Config, handle: &objectiveai_sdk::cli::output::Handle) -> Result<(), crate::error::Error> {
        ...
        match self {
            Commands::Get { id, message_index, media_index } => {
                let content = client.read_agent_completion_message_video(&id, message_index, media_index).await.map(objectiveai_sdk::filesystem::logs::LogContent::video)?;
                {
            ...
            Commands::Subscribe { id, message_index, media_index, timeout_ms, require_modification } => {
                ...
                {
                match result.map(objectiveai_sdk::filesystem::logs::LogContent::video) {
                    Some(content) => {
                        crate::log_line::emit_log_content(content, handle).await;
                        Ok(())
                    }
