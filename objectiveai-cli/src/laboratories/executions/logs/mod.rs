...
impl Commands {
    pub async fn handle(self, cli_config: &crate::Config) -> Result<crate::Output, crate::error::Error> {
        ...
        match self {
            Commands::Get { id } => {
                let content = objectiveai::filesystem::logs::client::read_laboratory_execution(&client, &id).await.map(objectiveai::filesystem::logs::LogContent::Json)?;
                Ok(crate::Output::LogsGet(content))
            }
            Commands::Subscribe { id, timeout_ms, require_modification } => {
                let result = objectiveai::filesystem::logs::client::subscribe_laboratory_execution(&client, &id, std::time::Duration::from_millis(timeout_ms), require_modification).await;
