...
//! env var is unset, every `cli_log!` invocation is a no-op (the
//! formatter never runs, no allocations).
//!
//! Format: `CLI t=<unix_ms> tag=<event_tag> [k=v]*` — one line per
//! call, mutex-serialized so concurrent writers don't interleave.
//! The leading `CLI` is the component prefix; sibling crates use
//! `PROXY` and `API`. Sorting the merged file by the `t=` field
//! gives a chronological cross-layer timeline.
//!
//! Intentionally cheap to remove: nothing depends on this module
//! except the explicit `cli_log!` call sites and the
//! `#[macro_use] mod diagnostic_log;` declaration in the parent.
...
macro_rules! cli_log {
    ($tag:literal $(, $field:ident = $value:expr)* $(,)?) => {{
        if let Some(sink) = $crate::api::diagnostic_log::sink() {
            let line = format!(
                concat!("CLI t={} tag=", $tag, $(" ", stringify!($field), "={:?}"),*),
                $crate::api::diagnostic_log::now_ms()
                $(, $value)*
            );
