...
class SwarmBaseAgentsItemVariant1(AgentOpenrouterAgentBase):
...
class SwarmBaseAgentsItemVariant2(AgentClaudeAgentSdkAgentBase):
    count: int = Field(1, description='Number of instances of this agent in the swarm. Defaults to 1.', ge=0, le=18446744073709551615)
    fallbacks: Optional[list[AgentBase]] = Field(None, description='Fallback agents to try if the primary fails.')


class SwarmBaseAgentsItemVariant3(AgentMockAgentBase):
    count: int = Field(1, description='Number of instances of this agent in the swarm. Defaults to 1.', ge=0, le=18446744073709551615)
    fallbacks: Optional[list[AgentBase]] = Field(None, description='Fallback agents to try if the primary fails.')


class SwarmBaseAgentsItem(RootModel):
...
along with fallback agents to try if the primary fails."""
    ...
    root: Union[SwarmBaseAgentsItemVariant1, SwarmBaseAgentsItemVariant2, SwarmBaseAgentsItemVariant3]
...
and sorted when converting to [`Swarm`]."""
    ...
    agents: list[SwarmBaseAgentsItem] = Field(..., description='The LLMs in this swarm, with optional counts and fallbacks.')
