#!/usr/bin/env bash
# Builds objectiveai-mcp and places the binary in embed/<target>/<profile>/.
# Skips the build if the source fingerprint hasn't changed.
# All arguments are forwarded to cargo build.
# Output is captured to .logs/build/objectiveai-mcp.txt.
#
# Usage:
#   bash objectiveai-mcp/build.sh [--release] [--target <triple>] [...]
