...
func (v *ConfigViewerSecretSignaturePair) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"secret", "signature"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("ConfigViewerSecretSignaturePair: missing required field %q", key)
		}
	}
	type Alias ConfigViewerSecretSignaturePair
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = ConfigViewerSecretSignaturePair(alias)
	return nil
}
