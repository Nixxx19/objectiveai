// THIS FILE IS AUTO-GENERATED. DO NOT EDIT.
// Generated by ObjectiveAI.CodeGen from objectiveai-json-schema/.

using ObjectiveAI;
using ObjectiveAI.Agent.Completions.Response;
using ObjectiveAI.Attributes;
using ObjectiveAI.Functions.Expression;
using System.ComponentModel;
using System.Text.Json.Serialization;

namespace ObjectiveAI.Functions.Executions.Response.Unary;

/// <summary>
/// A complete function execution response (non-streaming).
/// </summary>
[Description("A complete function execution response (non-streaming).")]
[JsonSchemaTitle("functions.executions.response.unary.FunctionExecution")]
public class FunctionsExecutionsResponseUnaryFunctionExecution
{
    /// <summary>
    /// Unix timestamp when the execution was created.
    /// </summary>
    [Description("Unix timestamp when the execution was created.")]
    [JsonPropertyName("created")]
    [JsonSchemaRange(Minimum = "0", Maximum = "18446744073709551615")]
    public ulong Created { get; set; } = default!;

    /// <summary>
    /// Error details if the execution failed.
    /// </summary>
    [Description("Error details if the execution failed.")]
    [JsonPropertyName("error")]
    [JsonSchemaNullable]
    public ResponseError? Error { get; set; } = null;

    /// <summary>
    /// ID of the function used (if remote).
    /// </summary>
    [Description("ID of the function used (if remote).")]
    [JsonPropertyName("function")]
    [JsonSchemaNullable]
    public string? Function { get; set; } = null;

    /// <summary>
    /// Unique identifier for this execution.
    /// </summary>
    [Description("Unique identifier for this execution.")]
    [JsonPropertyName("id")]
    public string Id { get; set; } = default!;

    /// <summary>
    /// Object type identifier.
    /// </summary>
    [Description("Object type identifier.")]
    [JsonPropertyName("object")]
    public FunctionsExecutionsResponseUnaryObject Object { get; set; } = default!;

    /// <summary>
    /// The final output (scalar or vector score).
    /// </summary>
    [Description("The final output (scalar or vector score).")]
    [JsonPropertyName("output")]
    public IFunctionsExpressionTaskOutputOwned Output { get; set; } = default!;

    /// <summary>
    /// ID of the profile used (if remote).
    /// </summary>
    [Description("ID of the profile used (if remote).")]
    [JsonPropertyName("profile")]
    [JsonSchemaNullable]
    public string? Profile { get; set; } = null;

    /// <summary>
    /// Reasoning summary if reasoning was enabled.
    /// </summary>
    [Description("Reasoning summary if reasoning was enabled.")]
    [JsonPropertyName("reasoning")]
    [JsonSchemaNullable]
    public FunctionsExecutionsResponseUnaryReasoningSummary? Reasoning { get; set; } = null;

    /// <summary>
    /// Token for retrying this execution with cached votes.
    /// </summary>
    [Description("Token for retrying this execution with cached votes.")]
    [JsonPropertyName("retry_token")]
    [JsonSchemaNullable]
    public string? RetryToken { get; set; } = null;

    /// <summary>
    /// Results from each task in the function.
    /// </summary>
    [Description("Results from each task in the function.")]
    [JsonPropertyName("tasks")]
    public List<IFunctionsExecutionsResponseUnaryTask> Tasks { get; set; } = default!;

    /// <summary>
    /// Whether any tasks encountered errors.
    /// </summary>
    [Description("Whether any tasks encountered errors.")]
    [JsonPropertyName("tasks_errors")]
    public bool TasksErrors { get; set; } = default!;

    /// <summary>
    /// Aggregated token and cost usage.
    /// </summary>
    [Description("Aggregated token and cost usage.")]
    [JsonPropertyName("usage")]
    public AgentCompletionsResponseUsage Usage { get; set; } = default!;
}
