...
//! env var is unset, every `sdk_log!` invocation is a no-op (the
//! formatter never runs, no allocations).
//!
//! Format: `SDK t=<unix_ms> tag=<event_tag> [k=v]*` — one line per
//! call, mutex-serialized so concurrent writers don't interleave.
//! The leading `SDK` is the component prefix; sibling crates use
//! `PROXY`, `API`, and `CLI`. Sorting the merged file by the `t=`
//! field gives a chronological cross-layer timeline.
//!
//! Intentionally cheap to remove: nothing depends on this module
//! except the explicit `sdk_log!` call sites and the
...
macro_rules! sdk_log {
    ($tag:literal $(, $field:ident = $value:expr)* $(,)?) => {{
        if let Some(sink) = $crate::diagnostic_log::sink() {
            let line = format!(
                concat!("SDK t={} tag=", $tag, $(" ", stringify!($field), "={:?}"),*),
