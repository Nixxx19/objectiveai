...
//! stdout, the host runs the command and pipes every emitted
//! [`Output`] back into the plugin's stdin — wrapped in one of these
//! envelopes per line. The envelope is uniform whether the
//! dispatching `Command` carried an `id` or not, so the plugin's
//! read loop parses the same shape for every line.
//!
