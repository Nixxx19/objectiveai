...
# objectiveai-mcp-proxy build (proxy + objectiveai-rs + Cargo.lock).
#
# Usage:
...
compute_fingerprint() {
  {
    ...
    # Proxy's own source — exclude the test-upstream and tests-ts
    # subdirectories since neither is part of the binary.
    find "$SCRIPT_DIR/src" -type f -name '*.rs' | sort
    echo "$SCRIPT_DIR/Cargo.toml"
    # Workspace dep: objectiveai-rs (mcp feature).
