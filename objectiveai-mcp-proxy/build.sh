#!/usr/bin/env bash
# Builds objectiveai-mcp-proxy and places the binary in
# embed/<target>/<profile>/. Skips the build if the source fingerprint
# hasn't changed. All arguments are forwarded to cargo build.
# Output is captured to .logs/build/objectiveai-mcp-proxy.txt.
#
# Default target is the host triple. The eventual embedder of this
# binary should pass `--target <whatever-it-builds-against>` so the
# embedded binary matches the embedder's own platform — this proxy is
# meant to ship as a sidecar process, not a Docker-injected musl
# artifact like objectiveai-mcp-filesystem.
#
# Usage:
#   bash objectiveai-mcp-proxy/build.sh [--release] [--target <triple>] [...]
