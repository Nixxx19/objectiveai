//! Caching wrapper for swarm fetchers.

use crate::ctx;
use futures::FutureExt;
use std::sync::Arc;

/// Wraps an swarm fetcher with per-request deduplication caching.
///
/// When multiple parts of a request need the same swarm, this fetcher
/// ensures only one actual fetch is performed. Subsequent requests for the
/// same swarm ID within the same request context share the result.
#[derive(Debug, Clone)]
...
{
    /// Fetches an swarm, using the request-scoped cache for deduplication.
    ///
