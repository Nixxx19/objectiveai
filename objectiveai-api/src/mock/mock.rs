...
fn get_agent_json(name: &str) -> Option<&'static str> {
    match name {
        ...
        "instruction-10x-tools" => Some(include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/assets/mock/agents/instruction-10x-tools.json"))),
        _ => None,
    }
}

/// All mock Agent names.
const AGENT_REPOSITORIES: &[&str] = &["schema-logprobs", "instruction", "tool-call", "instruction-logprobs", "invention", "error-probability-50", "instruction-10x-tools"];
...
fn get_swarm_json(name: &str) -> Option<&'static str> {
    match name {
        ...
        "twenty-agents-10x-tools" => Some(include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/assets/mock/swarms/twenty-agents-10x-tools.json"))),
        _ => None,
    }
}

/// All mock Swarm names.
const SWARM_REPOSITORIES: &[&str] = &["schema-and-tool", "instruction-duo", "twenty-agents-10x-tools"];
