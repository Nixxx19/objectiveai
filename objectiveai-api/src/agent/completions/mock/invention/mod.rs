...
fn walk_object_fields(
    ...
    // Include ALL properties (not just required), because media fields must be
    // referenced in expressions even if technically optional. The validator
    // (AV18/AS20) checks all declared properties for multimodal types.
    let field_names: Vec<String> = {
        // Start with required fields in order, then add any remaining properties
