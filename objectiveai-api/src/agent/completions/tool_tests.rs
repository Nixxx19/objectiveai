...
fn mcp_tool(name: &str) -> crate::mcp::tool::Tool {
    crate::mcp::tool::Tool {
        name: name.into(),
        title: None,
        description: None,
...
fn test_single_mcp_tool() {
    let conn = crate::mcp::Connection::new_for_test("server-a".into(), "https://a.com/mcp".into());
...
fn test_multiple_mcp_no_conflicts() {
    let conn1 = crate::mcp::Connection::new_for_test("alpha".into(), "https://a.com/mcp".into());
    let conn2 = crate::mcp::Connection::new_for_test("beta".into(), "https://b.com/mcp".into());
...
fn test_mcp_conflict_same_server_name_different_urls() {
    let conn1 = crate::mcp::Connection::new_for_test("myserver".into(), "https://a.com/mcp".into());
    let conn2 = crate::mcp::Connection::new_for_test("myserver".into(), "https://b.com/mcp".into());
    let tools1 = Arc::new(vec![mcp_tool("search")]);
    let tools2 = Arc::new(vec![mcp_tool("search")]);
    let (names, map) = resolve_tools(&[conn1, conn2], &[tools1, tools2], None, None);
    assert_eq!(names.len(), 2);
...
fn test_invention_conflicts_with_mcp() {
    let conn = crate::mcp::Connection::new_for_test("alpha".into(), "https://a.com/mcp".into());
...
fn test_mixed_no_conflicts() {
    let conn = crate::mcp::Connection::new_for_test("server".into(), "https://s.com/mcp".into());
