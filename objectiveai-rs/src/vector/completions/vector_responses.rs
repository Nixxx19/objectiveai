...
pub fn into_parts_for_prompt(
...
) -> Vec<crate::chat::completions::request::RichContentPart> {
    ...
    for (i, (vector_pfx_key, vector_response_index)) in vector_pfx_indices.iter().enumerate() {
        ...
        match &vector_responses[*vector_response_index] {
            ...
            crate::chat::completions::request::RichContent::Parts(rich_parts) => {
                for rich_part in rich_parts {
                    match rich_part {
                        crate::chat::completions::request::RichContentPart::Text { text } => {
                            parts.push(
                                crate::chat::completions::request::RichContentPart::Text {
                                    text: json_escape::escape_str(text).to_string(),
                                },
