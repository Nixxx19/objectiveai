...
pub fn run_jq(value: &impl Serialize, filter: &str) -> Result<Vec<serde_json::Value>, super::super::Error> {
    let program = load::File { code: filter, path: () };
    let loader = load::Loader::new(jaq_std::defs().chain(jaq_json::defs()));
    let arena = load::Arena::default();

    let modules = loader.load(&arena, program)
        .map_err(|errs| super::super::Error::JqParse(
    ...
    let filter = Compiler::default()
        ...
        .compile(modules)
        .map_err(|errs| super::super::Error::JqCompile(
            errs.into_iter().map(|e| format!("{e:?}")).collect::<Vec<_>>().join(", ")
        ))?;
    ...
    for item in out {
        match item {
            ...
            Err(err) => return Err(super::super::Error::JqRuntime(format!("{err:?}"))),
        }
    }
    Ok(results)
}
