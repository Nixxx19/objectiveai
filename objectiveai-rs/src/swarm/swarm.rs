...
/// The base configuration for an Swarm (without computed ID).
///
/// Contains a list of agent configurations that will be validated, deduplicated,
/// and sorted when converting to [`Swarm`].
...
impl TryFrom<SwarmBase> for Swarm {
    type Error = String;
    fn try_from(
        SwarmBase {
            agents: base_agents,
...
impl Swarm {
    /// Converts an SwarmBase to Swarm while aligning profile weights.
    ///
    /// Profile weights are filtered (count-0 removed), merged (weighted avg by count),
    /// and sorted to match the resulting Swarm's agent ordering.
    pub fn try_from_with_profile(
        ...
        Ok((Swarm { id, agents }, Profile::Entries(entries)))
    }
}
