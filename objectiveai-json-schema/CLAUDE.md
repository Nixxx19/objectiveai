...
The builder (`builder/src/main.rs`) calls `objectiveai::json_schemas()`, normalizes each schema, orders keys canonically, and writes one file per type. It clears all `.json` files in the output directory on each run (preserving `builder/`, `CLAUDE.md`, and other non-JSON files).
